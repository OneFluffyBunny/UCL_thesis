...
`newman_q`/`normalized_qm` DISCOVER a partition; `planted_q` (EXPERIMENTAL)
...
separate modules?" -- which needs no search and so has no optimiser to fail:
    ...
    qm.planted_q(G, {i: i // 4 for i in range(8)}, exclude=[8])
...
from .graph import (describe, from_blocks, from_matrix, layered_mask, role_mask,
                    roles_for)
from .metrics import (newman_q, normalized_qm, partition, planted_q,
                      role_segregation, threshold_sweep)
from .plot import open_file, plot_communities

__all__ = ["describe", "from_blocks", "from_matrix", "layered_mask", "role_mask",
           "roles_for", "newman_q", "normalized_qm", "partition", "planted_q",
           "role_segregation", "threshold_sweep", "plot_communities", "open_file"]
