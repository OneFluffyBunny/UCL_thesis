# Adapted from experiments/experiment_4/tasks.py (2026-08-19): identical except
# the repo-root path climbs one extra level, since necgp/ sits one directory
# deeper (experiments/experiment_6/necgp/ vs experiments/experiment_4/). See
# ../README.md.
"""Task shim for necgp -- loads the verified KA task, packs it to bitmasks.
...
This experiment is pure-Python by design, so it loads the former but exposes it
under the latter's name, since `retina_ka2005` is the name the rest of the
project uses for this rule. `shared_tasks.py`'s *other* retina -- the
...
one-module attractor that would fake a null on H1. See experiment_4/README.md.
...
_KA_TASKS_PY = pathlib.Path(__file__).resolve().parents[3] / "kashtan_alon" / "tasks.py"

_spec = importlib.util.spec_from_file_location("_ka_tasks_for_necgp", _KA_TASKS_PY)
