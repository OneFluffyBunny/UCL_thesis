"""Tests for the fake-module colouring in `visualize._box_style` (copied from necgp/).

necgp_pairwise note: this variant's `compress` cannot create a fake module, and its
`ecgp.validate` asserts none survived, so the fixtures below (which deliberately
contain fake modules) skip `validate`. The grey-out is kept as a safety net and so
drawings stay comparable with necgp/'s.
