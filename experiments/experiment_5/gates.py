"""Primitive function set for experiment 5 -- pure Python ints, no numpy, no JAX.
...
arbitrary-precision two's-complement, so a complement has to be confined to the
truth table's width.

`[exp5]` That confinement is written `x ^ mask`, not `~x & mask`. The two are equal
for any `x` inside the mask, but `~x & mask` allocates two full-width integers (the
negative complement, then the masked result) where `x ^ mask` allocates one. At 8
inputs a wire is 32 bytes and nobody cares; at 18 inputs it is 32 KB and the hot loop
is memory-bound, so halving the traffic through every NAND, NOR, XNOR and NOT is one
of the few optimisations that still bites in the big-brain regime.
