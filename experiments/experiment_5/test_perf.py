"""Performance and parallelism tests for experiment 5.
...
# `[exp5]` One ceiling per interpreter. PyPy is several times faster than CPython on
# this 8-input task (`bench.py`), so reusing CPython's numbers would make the PyPy run
# unable to fail: a 3x regression would still pass. The PyPy column is set the same
# way -- loose enough not to fire on scheduling noise, tight enough that losing the
# JIT trips it.
_CEILINGS = {
    "cpython": {100: 0.45, 400: 1.20, 800: 2.20},
    "pypy":    {100: 0.12, 400: 0.35, 800: 0.60},
}
MS_PER_GEN_CEILING = _CEILINGS["pypy" if hasattr(sys, "pypy_version_info")
                               else "cpython"]
...
def _time_seed(nodes: int, gens: int) -> float:
    ...
    t = time.time()
    train.run_seed(cfg, c, 0, RUNS, "perf")
...
def test_cpu_utilisation(nodes: int = 400, gens: int = 0) -> None:
    ...
    `[exp5]` Two changes, both forced by PyPy being fast enough to expose what the
    CPython numbers were hiding.

    `--no-stop-on-solution`. Every seed now runs the full budget, so all of them do
    IDENTICAL work. Previously a seed that solved at generation 3 000 finished while
    its neighbour ran to the end, and the pool sat waiting on the slowest -- load
    imbalance in the *search*, being reported as idleness in the *pool*. It stayed
    hidden at 6 000 CPython generations (2 of 12 seeds solved, so the spread was
    small) and became the dominant term the moment PyPy made a longer budget
    affordable: at 90 000 generations efficiency fell to 49% while the pool itself
    was working perfectly. Equal work per seed is the only way this measures what it
    claims to.

    Per-interpreter generation count. A worker's JIT warms from cold -- compiled
    traces do not survive a process spawn -- and each generation is ~4x cheaper, so
    PyPy needs a bigger job before a pool can pay for itself. The WORKLOAD is scaled
    rather than the threshold lowered: the threshold is the thing worth protecting.
    """
    if not gens:
        gens = 40000 if hasattr(sys, "pypy_version_info") else 6000
    ...
    rc = train.main(["--task", "retina_ka2005", "--operation", "and", "--no-viz",
                     ...
                     "--no-stop-on-solution",          # equal work per seed
