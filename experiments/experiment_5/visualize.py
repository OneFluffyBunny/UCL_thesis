...
# `[exp5]` A cone class is now a group NAME, and a task can have more than two
# groups (`retina_x3`, `add6`). `left`/`right` keep experiment 4's colours so its
# diagrams are unchanged; `g0..gK` fall back to a cycling palette, and `_cls_colour`
# is what every call site goes through so an unknown class can never KeyError mid-draw.
GROUP_CYCLE = ("#3498DB", "#E67E22", "#27AE60", "#8E44AD", "#C0392B", "#16A085")


def _cls_colour(name: str) -> str:
    if name in CLS_COLOUR:
        return CLS_COLOUR[name]
    if name.startswith("g") and name[1:].isdigit():
        return GROUP_CYCLE[int(name[1:]) % len(GROUP_CYCLE)]
    return NEUTRAL
...
def _render(ax, g, pheno: cgp.Phenotype, gates, n_in: int,
    ...
    for j in pheno.active:
        ...
        col = _cls_colour(pheno.cls[j]) if colour_cones else (mc or NEUTRAL)
    ...
    counts = pheno.counts()
    legend = [Patch(facecolor=_cls_colour(k), label=f"{k} ({counts[k]})")
              for k in tuple(pheno.names) + ("mixed", "const") if counts[k]] \
...
def _box_style(ind, j: int, gates, n_prim: int,
    ...
    never actually feed each other. Ported from `experiment_4/visualize.py` --
    same rule, split out so it is unit-testable without matplotlib.
    """
...
def frame_title(gen: int, goal: str, hits: int, max_hits: int,
                pheno: cgp.Phenotype, seed: int | None = None,
                alg: str = "CGP", mods: str = "") -> str:
    """The one-line caption every saved frame carries.

    `[exp5]` `max_hits` is `n_outputs * n_patterns`, not the pattern count: with
    several program outputs a perfect circuit scores one hit per (output, pattern)
    pair. See `cgp.max_hits`.
    ...
    tail = (f"{pheno.n_active} active nodes  |  "
            + "  ".join(f"{n} {c[n]}" for n in pheno.names)
            + f"  mixed {c['mixed']}")
    return (f"{head}  |  goal {goal.upper()}  |  "
            f"accuracy {hits / max_hits:.4f}  ({hits}/{max_hits})\n"
...
def stage_title(gen: int, hits: int, max_hits: int,
    ...
    """
    head = (f"gen {gen}  |  accuracy {100 * hits / max_hits:.2f}% "
            f"({hits}/{max_hits})")
