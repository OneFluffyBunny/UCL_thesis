...
FORKED FROM `experiment_4/ecgp.py` for the big-brain arm. Two `[exp5]` differences:
`fitness` scores MANY program outputs (see the function at the bottom), and the slot
draw it inherits from `cgp._draw_slots` is now draw-ordered, so a run is identical
under CPython and PyPy. Everything else -- every operator, every module rule -- is
...
def _apply(op: int, x: int, y: int, mask: int) -> int:
    ...
    `[exp5]` Complements are `x ^ mask`, matching `cgp.evaluate` and `gates.py`: one
    full-width allocation instead of the two that `~x & mask` needs.
    """
    if op == 0:
        return x & y
    if op == 2:
...
def fitness(ind: Individual, gates: Sequence[Gate], in_masks: Sequence[int],
    ...
    `[exp5]` `targets` is one mask per program OUTPUT, and scoring is delegated to
    `cgp.hits` / `cgp.balanced_score` so the two arms cannot drift apart: any change
    to what a hit means lands on both by construction.
    """
    outs = evaluate(ind, gates, in_masks, mask, n_in)
    h = cgp.hits(outs, targets, mask)
    if kind == "balanced":
        return cgp.balanced_score(outs, targets, mask), h
    return float(h), h
