...
Ported from `experiment_4/test_visualize.py`: same `_box_style` rule, same fixtures
(`Module`'s shape -- flat `conn`, no nesting -- is identical between exp4 and exp5).
`_box_style` is the one place the "grey out a fake module" rule lives; everything
else -- `_render_modular`, `draw_modular`, `stage_grid` -- just calls it.
"""
...
def test_box_style_greys_independent_parallel_gates_too() -> None:
    ...
    the module's own inputs, none reads another) must ALSO be drawn grey -- the
    case `is_trivial_module` does not catch but `is_fake_module` does.
    """
    parallel = ecgp.Module(mid=N_PRIM + 2, n_in=4, func=[NA, NA],
                           conn=[0, 3, 1, 2], out=[4, 5])
