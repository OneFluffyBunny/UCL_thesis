...
# ---------------------------------------------------------------------------
# pleiotropy  `[exp5 -- SPECIALISATION.md]`
# ---------------------------------------------------------------------------
#
# Espinosa-Soto & Wagner (2010) argue modularity arises by PLEIOTROPY REDUCTION:
# machinery shared between two demands obstructs adapting either one, so selection
# removes the sharing. They can only proxy that with Newman modularity on the
# interaction matrix. In a boolean circuit the quantity itself is computable exactly,
# which is the whole reason `SPECIALISATION.md` exists.
#
# NOT the hot path, by design. This is O(|active|**2) big-int ops and runs once per
# log row, never inside the (1+4) loop -- so `evaluate` is untouched and
# `test_equivalence.py`'s exp_4 <-> exp_5 equality cannot be disturbed by anything
# below.


def _forward(g: Genotype, ops: Sequence[int], arities: Sequence[int],
             vals: list[int], active: Sequence[int], mask: int, n_in: int,
             skip: int = -1) -> None:
    """Recompute `vals` for `active` (ascending == topological), in place.

    `skip` names one node whose value is held FIXED at whatever the caller put there
    -- that is the perturbation. Everything downstream of it recomputes normally,
    which is what makes the difference propagate.
    """
    a = g.arity
    conn, func = g.conn, g.func
    for j in active:
        if j == skip:
            continue
        b = j * a
        op = ops[func[j]]
        x = vals[conn[b]]
        y = vals[conn[b + 1]] if arities[func[j]] > 1 else 0
        if op == 0:
            v = x & y
        elif op == 1:
            v = x | y
...
def influence(g: Genotype, gates: Sequence[Gate], in_masks: Sequence[int],
              mask: int, n_in: int) -> dict[int, frozenset[int]]:
    """Active node index -> the program outputs whose BEHAVIOUR that node changes.

    Output `o` is in node `j`'s set iff inverting `j`'s truth-table mask (on every
    pattern at once) and repropagating changes `o`'s mask. Exact, and stronger than a
    cone test: a node can sit in an output's cone and have no influence on it at all
    (`x & ~x`, a wire masked off downstream, a module input the body ignores).

    ⚠️ This measures the influence of a **total** inversion, not of any single
    pattern. A node whose effect on `o` is self-cancelling under global inversion but
    not under a single-pattern flip would read as non-influential. Total inversion is
    chosen because it is what "this node is doing a job for that output" means at the
    level the fitness function sees, and because a per-pattern version is
    `2**n_in` times more work for a distinction no result here rests on.
    """
    ops = [gt.op for gt in gates]
    arities = [gt.arity for gt in gates]
    a = g.arity

    # backward pass -- the same reachability `evaluate` uses, written readably
    seen = bytearray(g.n_nodes)
    active: list[int] = []
    stack = [lbl - n_in for lbl in g.ogene if lbl >= n_in]
    while stack:
        j = stack.pop()
        ...
        active.append(j)
        b = j * a
        for k in range(arities[g.func[j]]):
            lbl = g.conn[b + k]
            if lbl >= n_in:
                stack.append(lbl - n_in)
    active.sort()

    base_vals = list(in_masks) + [0] * g.n_nodes
    _forward(g, ops, arities, base_vals, active, mask, n_in)
    base_out = [base_vals[lbl] for lbl in g.ogene]

    out: dict[int, frozenset[int]] = {}
    for j in active:
        vals = base_vals[:]
        vals[n_in + j] = base_vals[n_in + j] ^ mask          # the perturbation
        _forward(g, ops, arities, vals, active, mask, n_in, skip=j)
        moved = frozenset(o for o, lbl in enumerate(g.ogene)
                          if vals[lbl] != base_out[o])
        out[j] = moved
    return out


def specialisation(g: Genotype, gates: Sequence[Gate], in_masks: Sequence[int],
                   mask: int, n_in: int) -> tuple[float, int, int]:
    """(SPEC, n_specialised, n_pleiotropic) -- the statistic `SPECIALISATION.md` tests.

    A node is SPECIALISED if perturbing it moves exactly one program output, and
    PLEIOTROPIC if it moves more than one. Nodes that move none are excluded: they are
    inactive in the behavioural sense whatever the wiring says.

        SPEC = specialised / (specialised + pleiotropic)

    SPEC is `float('nan')` when no node influences anything (a circuit whose outputs
    are all wired straight to program inputs or to constants). NaN rather than 0.0 so
    it drops out of a median instead of dragging it down -- "undefined" and "fully
    pleiotropic" are different facts.

    ⚠️ SPEC is NOT comparable across tasks with different output structure: the task
    construction alone fixes most of it (two identical outputs force SPEC -> 0, two
    disjoint ones force SPEC -> 1). It is only meaningful compared BETWEEN ARMS THAT
    SHARE A TARGET -- see `SPECIALISATION.md` §2, which records that confound killing
    the first version of that design.
    """
    inf = influence(g, gates, in_masks, mask, n_in)
    spec = sum(1 for s in inf.values() if len(s) == 1)
    pleio = sum(1 for s in inf.values() if len(s) > 1)
    tot = spec + pleio
    return (float("nan") if tot == 0 else spec / tot), spec, pleio
