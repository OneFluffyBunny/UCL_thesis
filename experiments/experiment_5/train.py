"""CGP / ECGP (1+4) evolutionary strategy -- experiment 5 (big brains).

The loop is ../experiment_4/PAPER_SPEC.md section 3, verbatim:
...
# `[exp5]` NEITHER numpy NOR matplotlib IS IMPORTED AT MODULE LEVEL. This file has to
# import under PyPy, which has neither. Both were only ever used outside the search:
# numpy for `savez` and three summary statistics (`_mean`/`_std`/`_median` below
# replace them in a dozen lines of arithmetic), matplotlib for the diagrams. So the
# search runs headless on PyPy, and drawing stays a CPython job -- see `_require_viz`.
PYPY = hasattr(sys, "pypy_version_info")


def _viz():
    """Import `visualize` (and therefore matplotlib) on first use, never at import."""
    import visualize as viz_mod
    return viz_mod


def _require_viz(cfg: RunConfig) -> None:
    """Fail fast, and with the fix in the message, if diagrams are asked of PyPy."""
    if cfg.viz and PYPY:
        raise SystemExit("\n".join((
            "--viz needs matplotlib, which is not available under PyPy.",
            "  Run the search under PyPy with --no-viz (that is the fast path),",
            "  then redraw under CPython from the saved circuits:",
            "      .venv-pypy/Scripts/python train.py ... --no-viz --save-best",
            "      conda run -n lndp python render.py <run-dir>")))
...
def _std(xs) -> float:
    """Population standard deviation -- `np.std`'s default, so numbers stay
    comparable with experiment 4's summary lines."""
    xs = list(xs)
    if not xs:
        return 0.0
    m = _mean(xs)
...
def _median(xs) -> float:
    """`np.median`: the mean of the two middle values for an even-length sample."""
    xs = sorted(xs)
    n = len(xs)
    if not n:
        return 0.0
...
# `[exp5]` `left`/`right` survive for two-group tasks (so experiment 4's analysis
# scripts still read this file); `pure` is their K-group generalisation -- active
# nodes whose whole input cone lies inside ONE of the task's sub-problems. `out_pure`
# and `beh_pure` count PROGRAM OUTPUTS that decompose, structurally and behaviourally
# respectively: those two are the readout experiment 5 exists for, and they disagree
# exactly when a circuit is wired across sub-problems without being sensitive across
...
# Windows has no fork: every worker starts a fresh interpreter, which measures
# ~0.5-1 s each.
#
# `[exp5]` PyPy raises the bar twice over. Its interpreter starts slower, and -- the
# part that is easy to miss -- every worker has to warm its JIT up from cold, because
# compiled traces are per-process and are not inherited across a spawn. So a job that
# is worth parallelising under CPython can be a net loss under PyPy, which is exactly
# what `test_perf.py::test_cpu_utilisation` measured (64% efficiency on a job CPython
# ran at 77%). The budget is therefore per interpreter, not a constant.
_SPAWN_BUDGET_SECS = 25.0 if hasattr(sys, "pypy_version_info") else 8.0
...
# `[exp5]` ...and the same anchors measured under PyPy (`bench.py`, 8-input retina).
# Using the CPython numbers there would overestimate every job by ~4x and send short
# runs to a pool that cannot pay for itself.
_GEN_COST_MS_PYPY = ((100, 0.060), (400, 0.230), (800, 0.380))
...
def estimated_secs_per_gen(nodes: int) -> float:
    ...
    """
    pts = _GEN_COST_MS_PYPY if hasattr(sys, "pypy_version_info") else _GEN_COST_MS
    if nodes <= pts[0][0]:
        lo, hi = pts[0], pts[1]
    elif nodes >= pts[-1][0]:
        lo, hi = pts[-2], pts[-1]
...
# `[exp5]` Where PyPy stops paying, measured by `bench.py --crossover` (CGP, 200
# nodes, parityN sweep; numbers in RESULTS.md section 1): PyPy is 8.2x faster at 8
# inputs, 3.0x at 12, level at 14 (1.15x), and progressively SLOWER above -- 0.66x at
# 15, 0.20x at 18. The cause is not the search, it is the arithmetic: a wire is a
# `2**n_in`-bit integer, CPython's big-int bitwise ops are hand-tuned C, and once one
# op costs more than the interpreting of it, PyPy's only advantage is gone and its
# slower `rbigint` shows through.
#
# So "which interpreter" is a property of the TASK's width, not a preference, and
# getting it backwards costs up to 5x in either direction. Hence this advice line.
#
# The value is the first width at which CPython WINS, so 15 -- 14 is a tie and gets no
# advice in either direction worth acting on. Re-measure on a new machine; the
# constant is a measurement, not a law.
PYPY_CROSSOVER_INPUTS = 15


def interpreter_advice(n_in: int) -> str:
    """A one-line warning when the run is on the interpreter that loses for its width."""
    if PYPY and n_in >= PYPY_CROSSOVER_INPUTS:
        return (f"NOTE: {n_in} inputs is above the measured PyPy crossover "
                f"({PYPY_CROSSOVER_INPUTS}); CPython is expected to be FASTER here "
                f"(big-int bitwise ops dominate). See bench.py --crossover.")
    if not PYPY and n_in < PYPY_CROSSOVER_INPUTS:
        return (f"NOTE: {n_in} inputs is below the measured PyPy crossover "
                f"({PYPY_CROSSOVER_INPUTS}); PyPy is expected to be up to 8x faster "
                f"here. Try .venv-pypy/Scripts/python train.py ... --no-viz")
    return ""
...
def run_seed(cfg: RunConfig, ctx: dict, seed: int, out: pathlib.Path, run: str):
    """One independent run. Returns its result dict, or None if already finished.

    `[exp5]` Takes `_task_context`'s dict rather than nine positional pieces of it:
    the big-brain work added `n_out` and swapped `split` for `groups`, and threading
    each new field through three call sites by hand is exactly how an argument ends
    up transposed between the serial and the parallel path.
    """
    gate_set, n_in = ctx["gate_set"], ctx["n_in"]
    in_masks, mask = ctx["in_masks"], ctx["mask"]
    n_patterns, n_out = ctx["n_patterns"], ctx["n_out"]
    ...
    # is only meaningful when there are exactly two contiguous groups -- otherwise
    # the inputs are drawn as one block. Generalising the diagram to K groups is
    # deliberately deferred: an 800-node big-brain circuit is not legible anyway.
    viz_split = min(groups[1]) if groups and len(groups) == 2 else None
    ...
    n_mut = cgp.n_mutations(cfg.nodes, arity, n_out, cfg.mutation_rate)
    n_off = cfg.popsize - 1
    ...
    if cfg.ecgp:
        ...
        def new():
            return ecgp.random_individual(rnd, cfg.nodes, n_in, n_out, n_funcs)
        def offspring(par):
        ...
        def new():
            return cgp.random_genotype(rnd, cfg.nodes, n_in, n_out, n_funcs, arity)
        def offspring(par):
    ...
    def class_columns(view, pheno) -> dict:
        """`[exp5]` The structural + behavioural modularity columns for one log row.

        `left` / `right` are populated only for a two-group task, so experiment 4's
        readers keep working and a K-group task leaves them blank rather than lying
        about which group is which. `pure` is the K-group generalisation.

        `beh_pure` costs one extra `evaluate` per LOG ROW (not per generation), which
        is why it is affordable: the behavioural support of an output cannot be read
        off the wiring, it has to be measured from the output's truth table.
        """
        c = pheno.counts()
        pure = sum(c[n] for n in pheno.names)
        outs = cgp.evaluate(view, gate_set, in_masks, mask, n_in)
        beh = cgp.behavioural_classes(outs, in_masks, mask, n_in, groups)
        return dict(left=c.get("left", ""), right=c.get("right", ""), pure=pure,
                    mixed=c["mixed"], const=c["const"],
                    out_pure=pheno.n_pure_outputs(),
                    beh_pure=sum(1 for k in beh if k not in ("mixed", "const")))

    def log_row(gen: int) -> None:
        nonlocal t_interval
        view = as_cgp(parent)               # the effective circuit, modules inlined
        pheno = cgp.phenotype(view, n_in, gate_set, groups)
        c = pheno.counts()
        ...
        writer.writerow(dict(
            ...
            hits=p_hits, acc=round(p_hits / total_hits, 6),
        ...
        print(f"  [seed {seed:3d}] gen {gen:7d} | goal {goal:3s} | "
              f"hits {p_hits:4d}/{total_hits} ({p_hits / total_hits:.4f}) | "
              f"active {pheno.n_active:3d} "
              f"({' '.join(f'{n}{c[n]}' for n in pheno.names)} mixed{c['mixed']})"
        ...
        if draw_frames:
            ...
            viz_mod = _viz()
            ttl = viz_mod.frame_title(gen, goal, p_hits, total_hits, pheno, seed,
            ...
            if cfg.ecgp:
                viz_mod.draw_modular(parent, n_in, gate_set, n_funcs, path,
                                     title=ttl, split=viz_split)
            else:
                viz_mod.draw(view, pheno, gate_set, n_in, path, title=ttl, split=viz_split,
    ...
    for gen in range(start_gen, cfg.generations):
        ...
        if new_goal != goal:
            ...
            evals += 1
            # "Best" is a claim about a scoring rule that just changed, so it is
        ...
        if top > p_score:                                        # 4a
            ...
            if cfg.parsimony_tiebreak and len(ties) > 1:
                ...
                sizes = [cgp.phenotype(as_cgp(kids[i]), n_in, gate_set, groups).n_active
                         for i in ties]
                smallest = min(sizes)
        ...
        # ../experiment_4/PAPER_SPEC section 5: on promotion the module list is pruned to exactly the
        ...
        if p_hits > best_hits:
            best_geno, best_hits = parent.copy(), p_hits
        if solved_gen < 0 and p_hits == total_hits:
            solved_gen = gen
        ...
        # solved_gen is set above the instant p_hits first hits total_hits, so on
    ...
    if cfg.viz:
        ...
        if cfg.ecgp:
            ...
            viz_mod.draw(view, pheno, gate_set, n_in,
                         ...
                         title=ttl + "  |  flattened (modules inlined)", split=viz_split,
                         origin=org, colour_cones=cfg.colour_cones)
        else:
            viz_mod.draw(view, pheno, gate_set, n_in,
                         frames / f"seed{seed}_final.png", title=ttl, split=viz_split,
                         origin=org, colour_cones=cfg.colour_cones)
    ...
    if collect:
        ...
        for g_, goal_, hits_, snap in _even_picks(stages, n_panels):
            ...
            ph = cgp.phenotype(v, n_in, gate_set, groups)
            ...
            panels.append((snap if cfg.ecgp else v, ph,
                           viz_mod.stage_title(g_, hits_, total_hits, hidden), org))
        alg = "ECGP" if cfg.ecgp else "CGP"
        ...
        viz_mod.stage_grid(
            ...
            split=viz_split, colour_cones=cfg.colour_cones)
    ...
    if cfg.save_best:
        ...
        # `[exp5]` JSON, not `np.savez`: numpy does not exist under PyPy, and a
        # genotype is six lists of small ints -- a format numpy was never buying
        # anything for. `render.py` and any analysis script read it back with
        # `json.loads`.
        (out / f"{run}_seed{seed}_best.json").write_text(
            json.dumps(asdict(as_cgp(best_geno))), encoding="utf-8")
...
def _task_context(cfg: RunConfig):
    ...
    return dict(
        ...
        n_out=tasks_mod.n_outputs(cfg.task),
        n_patterns=tasks_mod.n_patterns(cfg.task),
        mask=tasks_mod.full_mask(cfg.task),
        in_masks=tasks_mod.input_masks(cfg.task),
        # `[exp5]` one target mask PER PROGRAM OUTPUT, per goal
        targets={op: tasks_mod.target_masks(cfg.task, op) for op in ops},
        # the task's own decomposition of its inputs; used only to colour and
        # classify nodes, never by the search itself
        groups=tasks_mod.input_groups(cfg.task),
    )
...
def _seed_task(payload):
    ...
    return run_seed(cfg, _task_context(cfg), seed, pathlib.Path(out_str), run)


def main(argv=None) -> int:
    cfg = parse(argv)
    _require_viz(cfg)                       # `[exp5]` fail before any work on PyPy
    ...
    print(f"experiment 5 -- {alg.upper()} | {name} | "
          f"{'PyPy' if PYPY else 'CPython'} {sys.version.split()[0]}")
    advice = interpreter_advice(n_in)
    if advice:
        print(f"  {advice}")
    print(f"  task {cfg.task} ({n_in} inputs, {n_out} outputs, {n_patterns} "
          f"patterns, {cgp.max_hits(n_out, n_patterns)} hits for a perfect score, "
          f"{len(ctx['groups'])} input group(s)) | "
    ...
    p_func = cfg.nodes / (cfg.nodes
                          + cfg.wiring_weight * (cfg.nodes * arity + n_out))
    print(f"  {cfg.nodes} nodes, {cgp.n_gene_slots(cfg.nodes, arity, n_out)} gene slots, "
    ...
    if workers == 1:
        summary = [run_seed(cfg, ctx, s, out, run) for s in seeds]
    else:
    ...
    line = (f"\ndone in {wall:.1f}s | best acc {_mean(accs):.4f} "
            f"+/- {_std(accs):.4f} (max {max(accs):.4f}) | "
            f"solved {len(solved)}/{cfg.n_seeds}")
    if solved:
        g = [s["solved_gen"] for s in solved]
        line += f" | median gens-to-solve {int(_median(g))}"
    print(line + f" | -> {out}")
