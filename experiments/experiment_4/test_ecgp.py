...
def test_fitness_matches_cgp_contract() -> None:
    ...
    gate_set, in_masks, mask = _ctx()
    target = tasks_mod.target_mask(TASK, "and")
    rnd = random.Random(6)
    for _ in range(30):
        ind = _evolve(rnd, gate_set, n_gen=30)
        a = ecgp.fitness(ind, gate_set, in_masks, target, mask, N_IN)
        b = cgp.fitness(ecgp.flatten(ind, N_IN), gate_set, in_masks, target, mask, N_IN)
