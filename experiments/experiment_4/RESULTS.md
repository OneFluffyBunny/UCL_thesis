...
> 🐛 **Cross-goal-maximum bug, found and fixed 2026-09-10. Affects MVG "best"
> numbers and MVG `_best.npz` genotypes only. All fixed-goal results are unaffected
> and were verified bit-identical. The search was never wrong.**
>
> **What was wrong.** `train.py:553-554` was `if p_hits > best_hits: best_geno,
> best_hits = parent.copy(), p_hits` — a single maximum carried across generations
> while the goal alternates underneath it. Under `mvg-and-or` that is not an
> accuracy: it maximises over two different targets, and the two are asymmetric (OR
> is true on far more patterns than AND, so it reaches a high count more easily), so
> the recorded "best" was decided largely by which goal happened to be live rather
> than by the lineage. It surfaced as `best_hits`/`best_acc` in `result.json` and
> `summary.csv`, and as the saved `_best.npz` genotype. Found while auditing after
> the same class of bug turned up in `kashtan_alon/` (see that file's own note).
>
> **What is NOT affected.**
> - The **search itself**: `train.py:481-487` already re-scores the parent against
>   the new target on every switch, so selection never used a stale fitness. Nothing
>   about evolution, RNG draws, mutation or selection changed — this experiment's
>   FROZEN, seed-identified reproducibility is intact.
> - **All fixed-goal runs.** A fixed goal has exactly one goal key, so the fix is a
>   no-op there. Verified empirically, not just reasoned: the pre-fix version was
...
>
> **What IS affected and needs re-deriving.** `scratch_analyze_mvg.py:51-52` and the
> MVG arms of `scratch_purity_table.py:93,114`, plus any MVG table below quoting
> "best acc" / "best hits med" (see the sections around lines 648 and 938). Scalars
> can be recomputed from the existing CSVs. **Genotypes cannot:** every MVG
> `_best.npz` on disk was written from the cross-goal peak, and the final parent was
> not persisted (the checkpoint is deleted on completion), so **MVG genotype-level
> analysis must wait for those runs to be regenerated by replay.** CGP is
> deterministic per seed, so that is an exact replay, not a new experiment.
>
> **The fix.** "Best" is now tracked **per goal** (`best_by_goal: goal -> (hits,
> genotype)`). Under MVG each goal gets its own file, `{run}_seed{k}_best_{goal}.npz`,
> and no bare `_best.npz` is written — a consumer must now say which goal it means,
> and an old ambiguous file can never be silently mistaken for a corrected one. The
> `best_hits`/`best_acc` summary scalar is the **worst goal's** best (the level the
> lineage demonstrably reached on *every* goal, which no easy goal can inflate), with
> new `best_goal` and `best_by_goal` fields naming it and showing the full per-goal
> picture. Fixed-goal output keeps the old filename, the old fields and the old
> values exactly.
>
> ⚠️ One side effect: the checkpoint format changed (`best`/`best_hits` → `best_by_goal`),
> so a checkpoint written mid-run *before* this fix cannot be resumed. Completed runs
> delete their checkpoints, so only an in-flight run is affected.

### Replay of the n50 MVG runs under the fix (2026-09-10) — and a ranking that flips

The `n50` MVG runs were replayed under the fixed code: `_purity_mvg` cgp n50 (4
seeds, 65 s wall at 4 workers) and ecgp n50 (4 seeds, 252 s). The n400 MVG runs
were **not** replayed and their numbers below are still the old contaminated ones —
they are flagged where they appear.

**The replay is provably faithful.** Two independent checks:
1. A full replay of cgp n50 seed 0 into a scratch directory, diffed against the
   then-untouched original: **402 rows × 15 substantive columns, 0 mismatches.**
   Same for `mvg50` ecgp seed 0. (Note the replay runs ~4× faster than the original
   wall-clock recorded in `result.json` — that is the "evaluate only active nodes"
   optimisation logged 2026-08-xx, which was behaviour-preserving, as this diff
   confirms.)
2. **The old `best_acc` reconstructs exactly as `max(new AND, new OR)` in all 8
   seeds.** That simultaneously *demonstrates* the bug (the old scalar really was a
   cross-goal maximum, not an accuracy) and proves the replay reproduced the original
   search — a changed search could not rebuild the old number to the bit.

| arm | seed | OLD `best_acc` (cross-goal max) | NEW on AND | NEW on OR | NEW `best_acc` (worst goal) |
|---|---:|---:|---:|---:|---:|
| CGP n50 | 0 | 0.9062 | 0.9062 | 0.9062 | 0.9062 |
| CGP n50 | 1 | 0.9062 | 0.8906 | 0.9062 | 0.8906 |
| CGP n50 | 2 | 0.9375 | 0.9375 | 0.9297 | 0.9297 |
| CGP n50 | 3 | 0.9062 | 0.8984 | 0.9062 | 0.8984 |
| **CGP mean** | | **0.9141** | | | **0.9062** |
| ECGP n50 | 0 | 0.9062 | 0.9062 | 0.8906 | 0.8906 |
| ECGP n50 | 1 | 0.9062 | 0.9023 | 0.9062 | 0.9023 |
| ECGP n50 | 2 | 0.9414 | 0.9414 | 0.8750 | 0.8750 |
| ECGP n50 | 3 | 0.9688 | 0.9688 | 0.9062 | 0.9062 |
| **ECGP mean** | | **0.9307** | | | **0.8936** |

⚠️ **This reverses the CGP-vs-ECGP ordering at n50 MVG, which is this experiment's
central comparison.** Old: ECGP 0.9307 vs CGP 0.9141, ECGP ahead by +0.0166. New:
CGP 0.9062 vs ECGP 0.8936, **CGP ahead by +0.0126**. The old ECGP lead was an
artifact: ECGP's two best seeds (0.9414, 0.9688) scored those on AND while sitting
at 0.8750 and 0.9062 on OR, and the cross-goal max reported only the flattering
half. ECGP is the arm that benefited most from the bug because its per-goal spread
is wider. n=4 either way, so neither ordering is statistically established — the
point is that the *previously reported* direction does not survive correction.

**Per-goal purity** (`scratch_purity_table.py`, now emitting one row per goal since
a circuit is only "best" *at* a goal):

| arm | purity (median) | active (median) | acc (median) |
|---|---:|---:|---:|
| CGP MVG 50n [and] | 0.744 | 9.5 | 0.902 |
| CGP MVG 50n [or] | 0.835 | 9.5 | 0.906 |
| ECGP MVG 50n [and] | 0.793 | 12.0 | 0.924 |
...
not deleted — they are kept as `*_best.npz.pre_goalfix.bak` beside the new per-goal
files.
...
(`kashtan_alon/RESULTS.md` Run 5: MVG 0.952 vs FG 0.904, both on AND — corrected
2026-09-10; the previously-quoted "MVG 0.975" was that project's own cross-goal
maximum, recorded on the easier OR goal) strictly degrades ours:
