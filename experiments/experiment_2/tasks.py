...
VERBATIM COPY of experiment_1/tasks.py — kept identical (not imported) so the two
experiments are guaranteed to train on exactly the same boolean problems while
staying self-contained. If you change one, change both.
...
to {-1,+1}. Targets are single bits in {0,1}.
"""
