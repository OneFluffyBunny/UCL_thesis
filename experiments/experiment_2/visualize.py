"""Visualisation / stats for a directly-encoded brain (experiment 2).

Mirrors experiment_1/visualize.py (inputs left, outputs right, hidden in a grid;
excitatory edges green, inhibitory slate, width by |weight|) so the two
experiments' brain images look the same and can be compared side by side.

The ONE difference: direct encoding has no evolved cell-types, so hidden neurons
are drawn in a single neutral colour instead of a per-type palette. If a
modularity metric later assigns each hidden neuron a community, this is the
natural place to recolour by community.
"""
