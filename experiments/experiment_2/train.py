...
def train_seed(brain_cfg, run_cfg, seed):
    ...
    Two champions, same rationale as experiment 1: ``best_*`` is the highest
    accuracy at ANY generation (under --mvg that peak belongs to whichever
    AND/OR phase happened to suit it, so it is not comparable across arms),
    while ``final_*`` is the best member of the LAST generation -- the endpoint
    of an equal number of generations in both arms, and therefore the one to
    measure for any FG-vs-MVG structural comparison.
    """
    key = jr.PRNGKey(seed)
