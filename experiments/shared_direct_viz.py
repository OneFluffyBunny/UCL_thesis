"""Shared visualisation / stats for a directly-encoded brain — single source.

Used (via a thin `visualize.py` shim) by every experiment that trains the
direct-encoding model: experiment 2 (CMA-ES) and experiment 3 (gradient descent).
Mirrors experiment_1/visualize.py (inputs left, outputs right, hidden in a grid;
excitatory edges green, inhibitory slate, width by |weight|) so every experiment's
brain images look the same and can be compared side by side.

The ONE difference from experiment 1: the direct encoding has no evolved
cell-types, so hidden neurons are drawn in a single neutral colour instead of a
per-type palette. If a modularity metric later assigns each hidden neuron a
community, this is the natural place to recolour by community.
"""
...
# --- colour scheme (identical reserved colours to experiment 1) --------------
INPUT_COLOR = "#4A90D9"   # blue
OUTPUT_COLOR = "#E74C3C"  # red
EXC_EDGE = "#2ECC71"      # green  (excitatory, +)
INH_EDGE = "#34495E"      # slate  (inhibitory, -)
HIDDEN_COLOR = "#9B59B6"  # purple (single colour -- no cell-types here)


def brain_stats(genome, cfg, threshold: float) -> dict:
    """Edge/density/sign statistics of the directly-encoded weight matrix
    (edges = |w| > thr). Same keys as experiment 1 minus the type counts."""
    ...
    return dict(w=w, n_edges=n_edges, max_edges=max_edges,
                density=density, n_exc=n_exc, n_inh=n_inh)
...
def visualize_brain(genome, cfg, threshold, save_path, title="", open_after=False):
    """Draw the directly-encoded brain to `save_path` (PNG). Returns the stats dict."""
    ...
    # node colours: inputs/outputs reserved, hidden all one neutral colour
    colors, labels = [], {}
    for i in range(n):
        if i < n_in:
            ...
            colors.append(HIDDEN_COLOR); labels[i] = f"h{i - n_in}"
    ...
    handles = [
        ...
        mpatches.Patch(color=HIDDEN_COLOR, label=f"Hidden ({n_hid})"),
